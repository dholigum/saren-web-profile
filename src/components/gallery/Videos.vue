<template>
  <v-container fluid class="pa-0 mt-4">
    <v-row class="text-center">
      <template v-for="(video, index) in paginatedVideo">
        <v-col class="my-3" :key="index">
          <v-card class="mx-auto" :width="responsiveCardWidth">
            <v-img contain class="white--text align-end" :height="responsiveImgHeight">
              <youtube
                :video-id="getYoutubeVidId(video.url)"
                :player-height="responsiveImgHeight"
                :player-width="responsiveCardWidth"
              ></youtube>
            </v-img>

            <v-card-subtitle
              class="pb-0 text-center black--text text-h6 font-weight-bold"
            >{{ video.title }}</v-card-subtitle>
            ...
            <v-card-actions>
              <v-btn color="orange" text target="_blank" :href="video.url">Buka di Youtube</v-btn>
            </v-card-actions>
          </v-card>
        </v-col>
      </template>
...
export default {
  ...
  computed: {
    ...
    paginationLength() {
      return Math.ceil(this.videos.length / this.pagination.perPage);
    },
    responsiveCardWidth() {
